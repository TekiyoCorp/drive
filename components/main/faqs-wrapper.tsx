...
export default async function FAQsWrapper() {
  ...
  try {
    ...
    // Skip fetch if no API token is configured
    if (!apiToken) {
      console.warn('Strapi API token not configured, using fallback FAQs');
      strapiFAQs = FAQ;
    } else {
    ...
    // Silently fall back to static data - don't log errors in production
    if (process.env.NODE_ENV === 'development') {
      console.warn('Error fetching FAQs, using fallback:', err instanceof Error ? err.message : 'Unknown error');
    }
